...
export default function CategoriasProduto({route,navigation})  {
    const baseURL ='https://eletrosom.com/shell/ws/integrador/listaProdutos?departamento='+route.params.item+'&version=15';
    ...
    function SearchBar () {
        ...
        return (
            <Appbar.Header style={{backgroundColor: '#1534C8',alignItems:'center',zIndex:99}}>
    ...
    const Options =() =>{
        ...
        return(
            <View style={{marginTop:-35,zIndex:1}}>
                <Appbar.Header style={{backgroundColor: '#fff',alignItems:'center'}}>
                    <Appbar.Content title= {Object.keys(data).length + ' Produtos'} style={{alignItems:'center'}} onPress={() => navigation.goBack()} />
                    <Appbar.Action icon={require('../assets/grade1.png')} onPress={() => setColumns( 1)} />
                    <Appbar.Action icon={require('../assets/grade.png')} onPress={() => setColumns( 2)}/>
        ...
        <SkeletonLoading visible={loading}>
            ...
            // onEndReachedThreshold={0.3}
              ...
              renderItem={({item})=>
                  <View >
                      {columns === 1
                          ?(<ListItem data={item} navigation={navigation} navigate={navigator}/>)
                          :(<ListItem2 data={item} navigation={navigation} navigate={navigator}/>)
                      }
                  </View> }
    ...
    return(
        ...
        <View style={{alignItems:"center",flex:1}}>
            {!data.emEstoque? (null):(<TouchableOpacity
                    ...
                    />
                    <View>
                        <Text style={{fontWeight:"bold",fontSize:13,maxWidth:260,width:'90%'}}>{data.nome.slice(0, 36)} </Text>
                        <View style={{width:80}}>
                            <StarRating
                                disabled={true}
                                maxStars={5}
