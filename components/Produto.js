...
export default function  Produto({route,navigation})  {
    ...
    const [sku1,setSku] = useState(sku2);
            ...
            <FlatList data={data}
                    ...
                    {item.filhos ?(
                        <TouchableOpacity  onPress={() => setModal(true)}>
                            <View style={{width:200,height:40,backgroundColor:'#F0F2F4',borderRadius:10,flexDirection:'row',padding:10,marginTop:24,marginBottom:24}}>
                            <Text>Voltagem: {filhos}</Text>
                            <Text style={{paddingLeft:180,paddingTop:10,position:'absolute'}}>
                                {'>'}
                            </Text>
                            </View>
                    ...
                    <Text style={{fontSize:20,width:'100%',textDecorationLine:'line-through'}}>R$ {precoDe}</Text>
                    <Text style={{fontWeight:"bold",fontSize:35,width:'100%',color: '#1534C8'}}>R$ {item.precoPor}</Text>
    
                       
                        {item.formaPagamento?(
                        <Text style={{fontWeight:"bold",fontSize:14.5,width:'100%',color: '#1534C8'}}>{item.formaPagamento}</Text>)
                        :(<Text></Text>)
                        }
                    ...
                    <View style={{flexDirection:'row',alignItems:'center'}}>
                        <View style={{width:70,alignItems:'center',backgroundColor:'#F0F2F4'}}>
                        <Text style={{fontSize:50}}>{!data.avaliacao ? (5) : (data.avaliacao)}</Text>
                        </View>
